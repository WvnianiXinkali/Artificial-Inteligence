...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in legal_actions:
            if self.getQValue(state, action) == optimalValue:
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        return sum(weights[ft] * vector[ft] for ft in vector)
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        difference = reward + self.discount * optimalValue - qValue
        vector = self.featExtractor.getFeatures(state, action)
        for ft in vector:
            self.weights[ft] += self.alpha * difference * vector[ft]
