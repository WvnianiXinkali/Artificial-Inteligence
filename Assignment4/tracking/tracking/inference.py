...
def constructBayesNet(gameState: hunters.GameState):
    ...
    distance_range = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1))
    variableDomainsDict[OBS0] = distance_range
    variableDomainsDict[OBS1] = distance_range
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            factors, joined_factor = joinFactorsByVariable(factors, variable)
            if len(joined_factor.unconditionedVariables()) > 1:
                factors.append(eliminate(joined_factor, variable))
        if len(factors) <= 0:
            return None
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            self.beliefs[position] *= self.getObservationProb(observation, pac_pos, position, jail_pos)
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            for newPos, prob in self.getPositionDistribution(gameState, pos).items():
                new_beleifs[newPos] += belief * prob
