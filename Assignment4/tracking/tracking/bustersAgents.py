...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for distribution in livingGhostPositionDistributions:
            ...
            ghost_poses.append(pos)
        closest_ghost_pos = min(ghost_poses, key=lambda pos: self.distancer.getDistance(pacmanPosition, pos))
        best_action = None
        min_dist= float('inf')
        for action in legal:
            pacToGhost = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), closest_ghost_pos)
            if pacToGhost < min_dist:
                min_dist = pacToGhost
                best_action = action
        return best_action
        "*** END YOUR CODE HERE ***"
